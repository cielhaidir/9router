...
export async function PUT(request, { params }) {
  try {
    ...
    if (isActive !== undefined && typeof isActive !== "boolean") return NextResponse.json({ error: "isActive must be boolean" }, { status: 400 });
    if (name !== undefined && (typeof name !== "string" || !name.trim())) return NextResponse.json({ error: "Invalid name" }, { status: 400 });
