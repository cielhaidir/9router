 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, onUsageRecorded, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      const usageRecord = await saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, silent: true });
      if (usageRecord?.id && onUsageRecorded) await onUsageRecorded({ usageHistoryId: usageRecord.id, tokens: usage, provider, model, requestId: jsonResponse.id || null });
  ...
  try {
    ...
    appendLog({ tokens: usage, status: "200 OK" });
    const usageRecord = await saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, silent: true });
    if (usageRecord?.id && onUsageRecorded) await onUsageRecorded({ usageHistoryId: usageRecord.id, tokens: usage, provider, model, requestId: parsed.id || null });
    if (log?.line) log.line(reqTag, "📊", formatDoneLine({ usage, latency: { total: Date.now() - requestStartTime } }));
